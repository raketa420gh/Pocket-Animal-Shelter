...
public static class Serializer
{
    ...
    public static T DeserializeFromPDP<T>(string fileName, SerializeType serializationType = SerializeType.Binary,
        string secureKey = "", bool logIfFileNotExists = true) where T : new()
    {
    ...
    /// <param name="serializationType">Type of serrialization.</param>
    /// <returns>Deserialized object if file exists or new instance if doesn't.</returns>
    public static T DeserializeFromResourses<T>(string fileName, SerializeType serializationType = SerializeType.Binary)
        where T : new()
    {
        switch (serializationType)
        {
    ...
    /// <param name="serializationType">Type of serrialization.</param>
    /// <returns>Deserialized object if file exists or new instance if doesn't.</returns>
    public static T DesirializeFromPath<T>(string directoryPath, string fileName,
        SerializeType serializationType = SerializeType.Binary) where T : new()
    {
        switch (serializationType)
        {
            case SerializeType.Binary:
        ...
        else
        {
            if (logIfFileNotExists)
            {
                Debug.LogError("File : \"" + fileName +
                               "\" does not exist at Resources folder. Maybe file located at subsidiary folder.");
            }
    ...
    public static T XmlDeserializeFromResourses<T>(string fileName, bool logIfFileNotExists = true) where T : new()
    {
        ...
        else
        {
            ...
            {
                Debug.LogError("File : \"" + fileName +
                               "\" does not exist at Resources folder. Maybe file located at subsidiary folder or there is extention in the end of file name.");
            }
        ...
        /// <param name="absolutePath">Absolute path to file(including file name and extention.</param>
        /// <returns>Deserialized object if file exists or new instance if doesn't.</returns>
        public static T JsonDeserializeFromPath<T>(string absolutePath, string secureKey = "", bool logIfFileNotExists =
 true) where T : new()
        {
            if (FileExistsAtPath(absolutePath))
            {
    ...
    public static void SerializeToPDP<T>(T objectToSerialize, string fileName,
        SerializeType serializationType = SerializeType.Binary, string secureKey = "")
    {
        switch (serializationType)
        {
            case SerializeType.Binary:
    ...
    /// <param name="fileName">Name of output file.</param>
    /// <param name="objectToSerialize">Reference to object that should be serialized.</param>
    public static void SerializeToResources<T>(T objectToSerialize, string fileName,
        SerializeType serializationType = SerializeType.Binary)
    {
        switch (serializationType)
        {
            case SerializeType.Binary:
    ...
    /// <param name="objectToSerialize">Reference to object that should be serialized.</param>
    /// <param name="serializationType">Type of serrialization.</param>
    public static void SerializeToPath<T>(T objectToSerialize, string directoryPath, string fileName,
        SerializeType serializationType = SerializeType.Binary)
    {
        switch (serializationType)
        {
